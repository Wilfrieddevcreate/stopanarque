  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm">
        ...
        {/* ── TEMPORAIRE — Premier déploiement uniquement ── */}
        <SetupSection />
      </div>
    </div>
  );
...
function SetupSection() {
  ...
  async function handleSetup(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setResult({ ok: true, message: `Compte créé pour ${data.email}. Connectez-vous maintenant.` });
        setOpen(false);
      }
    } catch {
  ...
  return (
    <div className="mt-8">
      {result && (
        <div className={`text-xs text-center mb-3 px-3 py-2 rounded-lg ${result.ok ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-700 border border-red-200"}`}>
          {result.message}
        </div>
      )}
      <button
        onClick={() => setOpen((v) => !v)}
        className="block w-full text-center text-xs text-muted/50 hover:text-muted transition-colors"
      >
        {open ? "▲ Fermer" : "Premier déploiement ?"}
      </button>
          ...
          >
            <div className="mt-3 bg-amber-50 border border-amber-200 rounded-xl p-4">
              ...
              <form onSubmit={handleSetup} className="space-y-3">
                ...
                />
                <input
                  type="email" required placeholder="Email admin" value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  className="w-full px-3 py-2 rounded-lg border border-amber-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-amber-300"
                />
                <input
                  type="password" required placeholder="Mot de passe (min. 8 car.)" value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  className="w-full px-3 py-2 rounded-lg border border-amber-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-amber-300"
                />
                <button
